...
//canvas constructor can have title;


/*
(check) Make cutoff 300 GeV. Everything past 300 GeV, the ratio is between the two intergals instead.
Investigate the failing datasets. Perhaps its the trees are empty, tree is named something. If tree fails, load tree path to a vector.

If it sounds reasonable, make sure to check before verifying

After this, calculate the MET from jet collection. Look at jets greater than 30 Gev. The vector sum of the Jet and the 2 Em object, grab its MET and record the MET.
Seperate EM objects from Jet collection be ensuring its delta R is greater than .4 from the jet . Get the transverse momentum (px and py) for all jet not including the Em Objects, and then subtract the EM Objects and subtract the EM Syspt. This means all momentum of the x and y momentum vectors,
not the magnitudes. Vector addition.

Hard Met is a poorer resolution compared to the default Met

(Check)//Divide by system PT. Get the ratio between the two. Photon/Electron. Ensure that the system PT between the weighte of the DiElectron System Match.

//Yohay wants snickers.
//Askew wants ginger mints.

//Allow overflow and underflow for ID

Check the invariant mass for all events and if both objects check ID.

Compare all to those that pass ID and to those that don't.

////////////////////////////////////////////////////////////////////////////////////////////////////////////////

(Check) Fix ID by including under and over flow values (-2 to 5); Apparently there is a overflow values for histograms.
Remake graphs to ensure that the photon and electron ID are valid.

(Check) ID comparison for invariant mass between passed and failed. Pass should be a fine peak and then opposite for failed.

(Try mass invariant ffailed being id = 0)

Remake ID applied graphs as well.

Compare all to those that pass ID and to those that don't.

Compared hard met to default met (sanity check)

(CHECK) Ratio between Jet System PT to ratio of Default MET

(CHECK) make a THF2 plot between MET for electron to Hard met of electron.

Ask Dr. Yohay and Dr. Ewald if they have preference dates.

hard MET.

(Check) FOR TH2F, extend overflow to 500 for x and y.

change pt to 40 or 50 for JETS selection. just curious here.

(Check) Invariant mass cut around Z->ee invariant mass
...
/*
Remake MET distribution applying ID

Reweighted DiPhoton/DiElectron System PT weights.

Reweighted Dielecton MET vs Diphoton MET

Check caluclation of outliers. Either High MET vs low hard MET or low MEt vs High hard MET.
